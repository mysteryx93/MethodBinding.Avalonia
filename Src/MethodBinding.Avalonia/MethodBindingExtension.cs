...
namespace MethodBinding.Avalonia
{
    ...
    {
        private static readonly List<AvaloniaProperty> s_storageProperties = new();
        ...
        private readonly List<AvaloniaProperty> _argumentProperties = new List<AvaloniaProperty>();
        ...
        public override object ProvideValue(IServiceProvider serviceProvider)
        {
            ...
            if (provideValueTarget.TargetObject is AvaloniaObject target && eventHandlerType != null) {
        ...
        private Delegate CreateEventHandler(AvaloniaObject source, Type eventHandlerType)
        {
            EventHandler handler = (sender, eventArgs) => {
                ...
                if ((methodName = arg0 as string) != null) {
                    if (source is StyledElement element) {
                        ...
                        Trace.TraceWarning($"[{nameof(MethodBindingExtension)}] Method target must be specified on element type '{source.GetType()}' because it is not a StyledElement and has no data context.");
                        return;
                    }
                }
                ...
                for (int i = methodArgsStart; i < _argumentProperties.Count; i++) {
                    ...
                    if (argValue is EventSenderExtension)
                        ...
                        argValue = eventArgsEx.GetArgumentValue(eventArgs, (source as StyledElement)?.Language);
        ...
        private static AvaloniaProperty SetUnusedStorageProperty(AvaloniaObject obj, object? value)
        {
            lock (s_storageProperties) {
                var property = s_storageProperties.Find(p => obj.GetValue(p) == AvaloniaProperty.UnsetValue);

                if (property == null) {
                    property = AvaloniaProperty.RegisterAttached<MethodBindingExtension, AvaloniaObject, object>("Storage" + s_storageProperties.Count);
